...
DB_URL = os.getenv("DATABASE_URL", "postgresql://payflow_admin:dummy_pass@postgres:5432/db_bank_hdfc")
...
async def debit_account(payload: TransactionPayload):
    ...
    async with app.state.pool.acquire() as connection:
        ...
        async with connection.transaction():
            ...
            # If two concurrent requests try to debit the same VPA, Postgres forces
            # one to wait. This mathematically prevents the double-spend problem.
            account = await connection.fetchrow(
            ...
            # Record the transaction. ON CONFLICT handles the narrow race window
